...
async def test_translation_performance_10_words() -> None:
    """10 words translate in <3 seconds (relaxed from <1s PRD target due to network latency + retries)."""
    translator = WordTranslator(source_language=Language.NL, target_language=Language.RU)
    ...
    assert elapsed < 3, f"Translation took {elapsed:.2f}s -- exceeds 3.00s QA gate"
