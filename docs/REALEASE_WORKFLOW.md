...
This document explains the reliable release process for `nl_processing`.
...
3. Create a local tag (do not push the tag via `git push origin <tag>`):

```bash
git tag v<VERSION>
```

4. Create the remote tag through GitHub API (this is the critical trigger):
...
5. Wait about 30 seconds and check the workflow run:

```bash
gh run list --workflow=publish.yml --limit 1
```

6. After the workflow succeeds, create a GitHub release for the existing tag:

```bash
gh release create v<VERSION> --title "v<VERSION>" --verify-tag --notes "..."
```

7. Verify PyPI has the new version:
...
- Keep versions consistent across git tag and `pyproject.toml`.
