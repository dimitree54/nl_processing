...
The sampler accepts an optional `scored_store` parameter conforming to `ScoredPairProvider` Protocol that requires only `get_word_pairs_with_scores() -> list[ScoredWordPair]`. When not provided, the sampler constructs `ExerciseProgressStore` directly.
