...
inputDocuments:
  ...
  - packages/database/docs/product-brief-database-2026-03-02.md
  - packages/database/docs/prd_database.md
workflowType: 'architecture'
project_name: 'nl_processing'
user_name: 'Dima'
date: '2026-03-07'
scope: 'database'
parentArchitecture: docs/architecture.md
---

# Architecture Decision Document — database

> For shared architectural decisions (core package, code style, patterns, project structure), see the [shared architecture](../../../docs/architecture.md). This document covers only module-specific decisions.
...
### Decision: Translator Is Injected, Not Constructed Internally

`database` does not construct `translate_word.WordTranslator` internally. If the caller wants fire-and-forget translation on `add_words()`, it passes a translator dependency into `DatabaseService`.

**Rationale:** this keeps `database` independently usable as its own package while still supporting higher-level orchestration flows.
...
├── models.py                    # AddWordsResult (+ re-exports of shared pair models)
...
- translation behavior with injected translator composition
