...
> For shared requirements (structured output, configuration, language support, error handling, NFRs), see the [shared PRD](../../../docs/prd.md). This document covers only module-specific requirements.
...
- **Optional fire-and-forget translation** composed by the caller when needed

## Success Criteria

### Technical Success

Technical success is measured by the outcomes in the table below.
### Measurable Outcomes
...
- **Remote latency:** optimized for correctness, not interactive latency (NFR1).
- **Exercise proliferation:** exercise types declared during store initialization.
- **Async translation failures:** failures logged without breaking caller's write path.
## User Journeys

### Journey 1: Persist New Vocabulary

Alex instantiates `DatabaseService(user_id="alex", translator=...)`, calls `add_words()`, and gets `AddWordsResult`. Background translation creates Russian words and links when a translator dependency is supplied.

### Journey 2: Read Durable Word Pairs

Alex calls `get_words()` and gets translated `WordPair` items from the authoritative remote store.

### Journey 3: Track Multiple Exercises Separately

Alex configures progress storage with `exercise_types=["nl_to_ru", "multiple_choice", "listen_and_type"]`. Each exercise type has its own dedicated score table.

### Journey 4: Support Local Cache Synchronization

`database_cache` calls internal APIs to export word pairs/scores and apply deltas idempotently using event IDs.
...
- FR9: New source-language words trigger asynchronous translation only when `DatabaseService` is composed with a translator dependency.
...
**Performance** — NFR1: `database` is optimized for durable remote correctness, not for interactive sub-200ms user-facing reads. NFR2: Batch operations should use database-native batching where practical. NFR3: Snapshot export and idempotent delta apply must be efficient enough for background cache refresh / flush jobs.

**Async** — NFR4: All public and cache-support methods are async. NFR5: Translation spawned by `add_words()` runs in the background. NFR6: Background failures are logged, not leaked to unrelated callers.

**Reliability** — NFR7: Remote connectivity failures surface as `DatabaseError`. NFR8: `database` remains the canonical source of truth even when caches are stale or unavailable. NFR9: Idempotent score replay guarantees safe retries from `database_cache`.

**Testing** — NFR10: Integration and e2e tests run against a real Neon database in the Doppler `dev` environment. NFR11: Tests verify per-exercise table creation and isolation. NFR12: Tests verify idempotent replay with repeated event IDs. NFR13: Tests leave the dev database clean after completion.

**Module-Specific Dependencies** — NFR14: `asyncpg` for Neon connectivity. NFR15: translation composition is optional and injected by the caller.
