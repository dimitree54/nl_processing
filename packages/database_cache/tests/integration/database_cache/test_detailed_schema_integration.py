...
async def test_schema_invalidation_with_real_deletion(tmp_path: Path) -> None:
    ...
    await local_store.upsert_cached_detail("boek", "noun", "nl_ru_noun", 1, json.dumps({"definition": "old_book"}))
    ...
    remote_record = DetailedWordRecord(
        ...
        schema_version=2,
        payload={"definition": "new_book", "gender": "neuter"},
    )
    ...
    assert json.loads(cached_after["payload"]) == {"definition": "new_book", "gender": "neuter"}
