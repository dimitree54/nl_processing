...
This extension adds a dedicated cache surface for the new tiered mixed-exercise mode while leaving the existing `DatabaseCacheService` and its current progress-report APIs unchanged. The preferred path is additive and isolated: introduce a tiered cache service with its own local schema and outbox semantics rather than widening the classic practice-cache contract.

### Tiered Functional Requirements

| ID | Requirement | Priority | Notes |
| --- | --- | --- | --- |
| TFR-DC-1 | The module must add a dedicated `TieredExerciseCacheService(user_id, source_language, target_language, mode_slug, exercise_types, cache_ttl, remote_tiered_progress?, local_store?, cache_dir?)` or equivalent additive API. | Must | Keeps the classic cache surface untouched. |
| TFR-DC-2 | Tiered warm-path reads must come entirely from local SQLite state and must not require a remote round trip after successful init/refresh. | Must | Same hot-path rule as the existing cache. |
| TFR-DC-3 | The tiered cache must persist local repeat-state in a dedicated table keyed by `(mode_slug, source_word_id)` where row presence means repeat mode is active. | Must | Mirrors the remote repeat-state table. |
| TFR-DC-4 | `record_tiered_result()` must update the local score overlay, local repeat-state, and pending tiered outbox event in one transaction before returning. | Must | Local state must become visible immediately. |
| TFR-DC-5 | `refresh()` must rebuild the local tiered snapshot from remote tiered data without losing acknowledged-but-unflushed local tiered results. | Must | Mirrors the existing snapshot-plus-outbox rule. |
| TFR-DC-6 | The tiered cache must expose a mixed progress summary where a word counts as positive only if every participating exercise score is `> 0`. | Must | Same aggregate rule as remote tiered progress. |
| TFR-DC-7 | Existing `DatabaseCacheService.get_progress_summary()` and current per-exercise cache progress reports must remain unchanged. | Must | User explicitly requested no regression in existing reports. |

### Tiered Rules and Invariants

- TBR-DC-1: Local tiered transition rules must match remote tiered transition rules exactly.
- TBR-DC-2: Missing participating scores are treated as `0`.
- TBR-DC-3: The same `mode_slug` must not be reused locally with a different exercise order.
- TBR-DC-4: A local repeat-state row with no positive participating score is invalid and must fail fast instead of being repaired silently.
- TBR-DC-5: Each pending tiered event must include `event_id`, `mode_slug`, `source_word_id`, `exercise_type`, and `delta`.

### Tiered Interfaces and Dependencies

| ID | Type | Direction | Counterparty | Contract or Data | Notes |
| --- | --- | --- | --- | --- | --- |
| TIF-DC-1 | Python API | Inbound | Callers, `sampling` | `init()`, `get_tiered_candidates()`, `get_tiered_progress_summary()`, `record_tiered_result()`, `refresh()`, `flush()`, `get_status()` | Additive tiered cache surface. |
| TIF-DC-2 | Remote sync | Outbound | `database` tiered store | `export_tiered_snapshot()` and `apply_tiered_result(...)` | Dedicated mixed-exercise remote contract. |
| TIF-DC-3 | Local storage | Internal | SQLite via `aiosqlite` | Tiered snapshot rows, tiered repeat-state rows, tiered metadata, and pending tiered events | Dedicated local schema for the additive tiered surface. |
| TIF-DC-4 | Change reference | Outbound | `sampling` | Local provider shape for tiered candidate reads | See the sampling tiered extension section. |

### Tiered Data and State Ownership

| Entity or State | Ownership | Description | Lifecycle or Retention | Notes |
| --- | --- | --- | --- | --- |
| Tiered cache file or schema | Owned | Durable local state for one `(user, source_language, target_language, mode_slug)` tiered instance. | Persists across restarts | Isolation is preferred over mutating the classic cache contract. |
| Cached tiered candidate rows | Owned | Local copy of translated pairs, ordered scores, and metadata required for tiered sampling. | Rebuilt on refresh | Dedicated to mixed-exercise flows. |
| Cached tiered repeat-state rows | Owned | Local mirror of remote repeat-mode rows keyed by `mode_slug` and `source_word_id`. | Updated on writes and refresh | Row presence means repeat mode is active. |
| Pending tiered events | Owned | Transactional outbox for replaying tiered answer results remotely. | Retained until flush succeeds | Needed for offline-safe tiered flows. |

### Tiered Processing Flow

1. `TieredExerciseCacheService.init()` opens or creates the local tiered cache, ensures the dedicated schema, and refreshes if no snapshot is present.
2. Tiered reads serve candidate rows and aggregate positive-progress summaries entirely from local state.
3. `record_tiered_result()` updates the answered exercise score, derives the next local repeat-state, writes a pending tiered event, and returns before flush completes.
4. `flush()` replays pending tiered events to the remote tiered store idempotently.
5. `refresh()` replaces the local snapshot from remote tiered data and reapplies pending tiered events so acknowledged local changes are not lost.

### Tiered Decisions

| ID | Decision | Status | Rationale | Consequence |
| --- | --- | --- | --- | --- |
| TDEC-DC-1 | Add a separate tiered cache surface instead of widening `DatabaseCacheService`. | Decided | Lowest-risk path for preserving current behavior. | Mixed-exercise callers opt into a new cache API. |
| TDEC-DC-2 | Mirror remote repeat-state locally as dedicated presence rows. | Decided | The state cannot be derived from scores alone. | Cache refresh and local writes must keep this mirror accurate. |
| TDEC-DC-3 | Keep tiered transition logic deterministic and identical on local write path and remote replay path. | Decided | Prevents drift between offline and online behavior. | Shared tests are required across cache and database. |

### Tiered Validation

**Acceptance Criteria:**

- TAC-DC-1: Existing `DatabaseCacheService` behavior and current progress reports remain unchanged.
- TAC-DC-2: Tiered warm-path reads stay local after init/refresh.
- TAC-DC-3: Tiered local writes update score, repeat-state, and outbox atomically and remain visible before flush.
- TAC-DC-4: Tiered refresh and flush preserve deterministic repeat-state and the aggregate "all scores positive" progress summary.

**Testing Strategy:**

- Unit: local transition matrix, invalid repeat-state detection, aggregate positive-progress math, and `mode_slug`/order validation.
- Integration: SQLite persistence of tiered snapshot, repeat-state rebuilds, outbox replay, and refresh overlay behavior.
- Contract: remote tiered snapshot and replay compatibility with the `database` tiered store.

### Tiered Risks

| ID | Risk | Impact | Mitigation or Next Step |
| --- | --- | --- | --- |
| TRISK-DC-1 | A dedicated tiered cache surface may duplicate some local snapshot data already held by the classic cache. | Local storage use can increase if both surfaces are enabled for the same user. | Prefer isolation first; revisit snapshot composition only if duplication becomes operationally relevant. |
| TRISK-DC-2 | Local and remote tiered state machines could drift. | Users could see one next exercise offline and another after sync. | Keep a shared transition matrix and parity tests across both modules. |

### Tiered Open Questions

| ID | Question | Status | Owner or Next Step | Notes |
| --- | --- | --- | --- | --- |
| TOQ-DC-1 | Should the long-term implementation reuse the classic practice-cache snapshot internally, or keep a dedicated tiered snapshot even if that duplicates local rows? | Open | Decide during implementation after measuring complexity vs storage trade-offs | The recommended first delivery keeps the tiered cache isolated. |
