...
`sampling` does not maintain its own state. It reads:

- The user's translated word pairs from `database`
- The user's per-exercise integer scores from `database`

**Rationale:** Sampling must reflect the latest persisted progress and must not diverge across processes. Keeping all progress state in the database makes sampling stateless and easy to scale.
...
├── __init__.py              # empty
├── service.py               # WordSampler (public class)
└── docs/
...
- **Unit tests:** Mock database access layer. Validate weight computation, aggregation across exercises, sampling without replacement, edge cases (empty candidates, limit <= 0).
